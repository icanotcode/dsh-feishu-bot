...
export async function installFeishuRuntime(ctx, config, client, tools, executeTool, options = {}) {
  ...
  let closing = false;
  const allowedUser = id => (config.authorizedUsers || []).find(user => user.openId === id);
  const authorized = id => !closing && Boolean(allowedUser(bindings.get(id)?.identity.openId));
  ...
  const register = tool => disposers.push(ctx.tools.register({ ...tool,
    ...
    async execute(args, exec) {
      ...
      if (userToolNames.includes(tool.name)) {
        ...
        const permissionPreset = allowedUser(binding.identity.openId)?.permissionPreset || config.permissionPreset;
  ...
  async function sessionFor(user, parsed, signal) {
    ...
    const allowed = allowedUser(user.identity.openId);
    const displayName = allowed?.displayName;
    if (!displayName) throw new Error('User authorization was revoked');
    const { agent } = await host.getOrCreate({ sessionId, workspacePath, title: `${displayName} · 飞书 · ${day()}`, agentPreset: config.agentPreset || 'standard', permissionPreset: allowed.permissionPreset || config.permissionPreset || 'workspace-write', ...(config.model ? { model: config.model } : {}) });
    binding.agent = agent;
    signal.throwIfAborted();
  ...
  async function receive(delivery, signal) {
    ...
    if (text === '/whoami') { await reply(parsed.messageId, `你的飞书 open_id：${parsed.senderId}\n请由本机管理员在插件授权名单中添加此 ID 和显示名称。`); return null; }
    if (!allowedUser(parsed.senderId)) { await reply(parsed.messageId, '你尚未获得使用权限。发送 /whoami 获取身份 ID，再联系本机管理员添加授权。'); return null; }
    const identity = { source: config.source, tenantId: parsed.tenantId || '', openId: parsed.senderId };
    const key = userKey(identity);
    if (!users.has(key)) users.set(key, { key, identity, store: history.forUser(identity) });
    const user = users.get(key);
    if (Buffer.byteLength(parsed.userText) > 512 * 1024) { await reply(parsed.messageId, '消息过长，请分成较短的消息发送。'); return null; }
    ...
    return serial(key, async () => {
      signal.throwIfAborted();
      if (!allowedUser(parsed.senderId)) return null;
      try {
        if (text === '/new') {
        ...
        try {
          agent.followup(createUserMessage({ content: [{ type: 'text', text: prompt }], source: { kind: 'webhook', provider: 'feishu', source: config.source, deliveryId: parsed.messageId, ruleId, form: 'notice', summary: 'Authorized Feishu conversation message' } }));
        ...
        if (!closing) await commandReply('消息未能进入会话，原文已保存。请在 Harness 检查模型、工作目录和授权配置后重新发送。');
      }
      return null;
    });
  }
