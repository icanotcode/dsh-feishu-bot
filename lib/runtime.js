...
import { createApprovals, approvalTool } from './approvals.js';
...
export async function installFeishuRuntime(ctx, config, client, tools, executeTool, options = {}) {
  ...
  const knownUsers = () => [...users.values()].map(u => ({ openId: u.identity?.openId, displayName: u.store.getProfile()?.displayName }));
  const approvals = createApprovals({ config, client, knownUsers });
  ...
  const isolatedTools = [...userTools, ...mediaToolDefinitions, readImageTool, capabilityTool, approvalTool, ...mailTools];
  ...
  const register = tool => disposers.push((membership ? membership.registerTool : value => ctx.tools.register(value))({ ...tool,
    ...
    async execute(args, exec) {
      ...
      if (isolatedToolNames.includes(tool.name)) {
        ...
        if (tool.name === 'feishu_approval') {
          exec.signal.throwIfAborted();
          return { data: await approvals.execute(args, binding) };
        }
  ...
  async function receive(delivery, signal) {
    ...
    return serial(key, async () => {
      ...
      try {
        ...
        const approvalReply = await approvals.handleCommand(text, parsed.senderId);
