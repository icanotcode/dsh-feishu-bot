...
export async function installFeishuRuntime(ctx, config, client, tools, executeTool, options = {}) {
  ...
  const isolatedTools = [...userTools, ...mediaToolDefinitions, readImageTool, ...mailTools];
    ...
    return serial(key, async () => {
      ...
      try {
        ...
        if (mailReply.handled) {
          ...
          if (!mailReply.resume) return null;
        }
        ...
        if (!resumedMail && text.startsWith('/')) { await commandReply('未知命令。发送 /help 查看支持的命令；普通消息会继续当前会话。'); return null; }
        let admitted;
        ...
        const requestText = resumedMail?.requestText ?? parsed.userText;
