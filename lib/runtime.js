  ...
  async function receive(delivery, signal) {
    ...
    return serial(key, async () => {
      ...
      try {
        ...
        const requestText = mailReply.discover ? '请调用 feishu_mail_discover 自动查找当前用户已提供邮箱的服务配置，然后向用户转述结果。无需再询问邮箱或要求用户填写服务器、端口、加密方式；找到服务后再询问授权码。只调用一次，不循环重试；此步骤不发送邮件。' : resumedMail?.requestText ?? parsed.userText;
