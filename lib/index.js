...
export class FeishuClient {
  ...
  async replyMessage(messageId, content, msgType = 'text', { uuid } = {}) {
    return this.request('POST', `/open-apis/im/v1/messages/${encodeURIComponent(messageId)}/reply`, {
      msg_type: msgType,
...
export function createFeishuEventReceiver(ctx, config) {
  ...
  return (payload, { authenticated = false } = {}) => {
    ...
    const appId = payload.header?.app_id || payload.app_id;
    if (config.expectedAppId && appId && appId !== config.expectedAppId) throw new WebhookHttpError(401, 'event belongs to a different Feishu application');
    const parsed = parseFeishuMessage(payload.event || payload);
    if (!parsed || !parsed.userText || parsed.sender.sender_type === 'app') {
      return;
    }
    parsed.tenantId = payload.header?.tenant_key || parsed.sender.tenant_key || '';
    if (typeof parsed.tenantId !== 'string') throw new WebhookHttpError(400, 'invalid tenant identity');
    ...
    if (admissions.has(eventId)) return admissions.get(eventId);
    const dispatch = ({ parsed: admitted, sensitive = false }) => {
      const delivery = {
        kind: 'feishu', source: WebhookSourceId(config.source), deliveryId: WebhookDeliveryId(eventId),
        event: { name: eventType, payload: sensitive ? { parsed: admitted } : { parsed: admitted, raw: payload.event || payload } },
        receivedAt: now,
      };
      try { ctx.webhookRuntime.dispatch(delivery); }
      catch {
        ctx.discardIngress?.(admitted);
    ...
    if (!ctx.prepareIngress) return dispatch({ parsed });
    // Only credential-state lookup and redaction happen before the fast ACK.
    // SMTP verification and Agent execution remain in the asynchronous runtime.
    const operation = Promise.resolve().then(() => ctx.prepareIngress(parsed)).then(dispatch).catch(() => {
      throw new WebhookHttpError(503, 'secure message admission is unavailable');
    });
    admissions.set(eventId, operation);
    void operation.finally(() => admissions.delete(eventId)).catch(() => {});
    return operation;
  };
}
...
export function createFeishuWebhookHandler(ctx, config, receiveEvent = createFeishuEventReceiver(ctx, config)) {
  return async (request, response) => {
    try {
      ...
      await receiveEvent(payload, { authenticated });
...
export const inject = [
  ...
  "sessionTitle", "workspaceRegistry", "sessionPersistence", "attachments", "llm"
];
...
export const Config = z.object({
  ...
  configFile: z.string(),
  botsFile: z.string(),
  historyRoot: z.string(),
  publicBaseUrl: z.string().default(""),
  tunnelProvider: z.union(["ngrok", "cloudflare", "custom"]).default("ngrok"),
  tunnelAutoRestart: z.boolean().default(false),
  ngrokTrafficPolicyFile: z.string().default(''),
  ngrokExecutablePath: z.string().default(''),
  cloudflareExecutablePath: z.string().default(''),
  cloudflareMode: z.union(['quick', 'named']).default('quick'),
  cloudflareTunnelName: z.string().default(''),
  cloudflareConfigFile: z.string().default(''),
  ...
  // Legacy setting only; runtime identity and confirmed names come from Feishu conversations.
  authorizedUsers: z.array(z.object({ openId: z.string().required(), displayName: z.string().required(), permissionPreset: z.union(['read-only', 'workspace-write']), roles: z.array(z.string()).default([]) })).default([]),
  // Capability bridge: default roles for skills without a feishu.json manifest.
  capabilityDefaultRoles: z.array(z.string()).default(['admin']),
  // Capability bridge: roles auto-granted to name-confirmed users not listed in authorizedUsers.
  defaultUserRoles: z.array(z.string()).default([]),
  dailyResetHour: z.number().step(1).min(0).max(23).default(4),
  dailyResetTimezone: z.string().default('Asia/Macau'),
  model: z.object({
...
export async function apply(ctx, config) {
  assertConfig(config);
  const hub = createRuntimeHub(ctx, { defaultSource: config.source });
  const manager = await createBotManager(ctx, config, {
    hub,
    createClient: (scope, botConfig) => new FeishuClient(scope, botConfig),
    installRuntime: (scope, botConfig, client, options) => installFeishuRuntime(scope, botConfig, client, feishuTools, executeFeishuTool, options),
    createReceiver: createFeishuEventReceiver,
    createHandler: createFeishuWebhookHandler,
    createTransport,
  });
  ctx.effect(() => () => manager.dispose(), 'feishu-bot: all robot connections and runtimes');
  ctx.provide('feishuBot', manager.default);
}
