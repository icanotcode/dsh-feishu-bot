...
export async function apply(ctx, config) {
  ...
  const management = await createManagement(ctx, config, feishuClient, {
    onConfigChanged: async () => { await runtime?.reconcile(); await transport?.reconcile(); },
  ...
  const disposeRule = await installFeishuRuntime(ctx, config, feishuClient, feishuTools, executeFeishuTool);
  runtime = disposeRule;
  ...
  await management.reconcileTunnel();
}
