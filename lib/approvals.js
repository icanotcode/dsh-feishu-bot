...
export function createApprovals({ config, client }) {
  ...
  return {
    ...
    adminNote(openId) {
      if (!isAdmin(openId)) return '';
      return '\n你是本机器人的管理员。用户权限审批由插件命令直接执行（不经过模型）：/approve <open_id> <角色> 批准、/reject <open_id> 拒绝、/pending 看待审批。收到审批请求或用户询问权限时，直接告知用户发送对应命令即可（命令由插件拦截执行，不经过你）；不要声称无法操作或引导用户去改配置文件。';
    },
