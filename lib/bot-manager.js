...
import { listBotProjects } from './project-catalog.js';
...
export async function createBotManager(ctx, baseConfig, dependencies) {
  ...
  async function start(slot) {
    ...
    try {
      ...
      const runtime = slot.runtime;
      slot.receive = createReceiver({ logger: ctx.logger,
        prepareIngress: async parsed => {
          const prepared = await runtime.prepareIngress?.(parsed) ?? { parsed, sensitive: false };
          if (!slot.accepting || slot.runtime !== runtime) {
            runtime.discardIngress?.(prepared.parsed);
            throw new Error('Bot ingress changed while preparing message');
          }
          return prepared;
        },
        discardIngress: parsed => runtime.discardIngress?.(parsed),
        webhookRuntime: {
        dispatch(delivery) {
  ...
  try {
    ...
    protect('/api/feishu-bot/projects', { GET: () => listBotProjects(ctx, service.list().bots) });
