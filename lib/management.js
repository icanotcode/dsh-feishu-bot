...
const PUBLIC_FIELDS = ['connectionMode', 'tunnelProvider', 'workspacePath', 'agentPreset', 'permissionPreset', 'ngrokDomain', 'publicBaseUrl', 'authorizedUsers', 'dailyResetHour', 'dailyResetTimezone'];
  ...
  const service = {
    ...
    async getTunnelStatus() {
      ...
      if (config.tunnelProvider === 'ngrok') {
        const detected = await service.getNgrokStatus();
        return { ...detected, ...common, state: detected.running ? 'detected' : 'not_detected', url: config.publicBaseUrl || detected.url };
      }
      return { ...common, state: config.publicBaseUrl ? 'configured' : 'unconfigured', running: null, url: config.publicBaseUrl || null,
        message: config.publicBaseUrl ? '公网地址已配置；尚未验证隧道运行状态或飞书回调可达性' : '请填写公网 HTTPS 根地址并保存；插件不会自动启动隧道' };
    },
    async getWebhookUrl() {
      if (config.connectionMode === 'websocket') return null;
      const base = config.publicBaseUrl || (config.tunnelProvider === 'ngrok' ? (await service.getNgrokStatus()).url : null);
