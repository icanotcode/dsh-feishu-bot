...
import { checkSetup, repairSetup } from './setup-check.js';
...
const PUBLIC_FIELDS = ['connectionMode', 'tunnelProvider', 'workspacePath', 'agentPreset', 'permissionPreset', 'ngrokDomain', 'publicBaseUrl', 'authorizedUsers', 'capabilityDefaultRoles', 'defaultUserRoles', 'dailyResetHour', 'dailyResetTimezone', 'tunnelAutoRestart', 'ngrokTrafficPolicyFile', 'ngrokExecutablePath', 'cloudflareExecutablePath', 'cloudflareMode', 'cloudflareTunnelName', 'cloudflareConfigFile'];
const TUNNEL_PATHS = ['ngrokTrafficPolicyFile', 'ngrokExecutablePath', 'cloudflareExecutablePath', 'cloudflareConfigFile'];
const TUNNEL_FIELDS = ['tunnelProvider', 'ngrokDomain', 'publicBaseUrl', 'tunnelAutoRestart', ...TUNNEL_PATHS, 'cloudflareMode', 'cloudflareTunnelName'];
const credentialRefs = new WeakMap();
const MAX_BODY = 64 * 1024;
const validateRoles = value => {
  if (!Array.isArray(value) || value.some(role => typeof role !== 'string' || !/^[a-z0-9-]{1,32}$/.test(role.trim()))) throw new ManagementError(400, '角色必须是小写字母、数字、短横线组成的数组（每项1–32字符）');
  return value.map(role => role.trim());
};
...
function validateSettings(input) {
  ...
  for (const key of PUBLIC_FIELDS) {
    ...
    if (key === 'capabilityDefaultRoles' || key === 'defaultUserRoles') {
      result[key] = validateRoles(input[key]);
      continue;
    }
    if (key === 'authorizedUsers') {
      ...
      result[key] = input[key].map(user => {
        ...
        if (user.roles !== undefined) result.roles = validateRoles(user.roles);
        return result;
      });
      continue;
    }
...
export async function createManagement(ctx, config, feishuClient, hooks = {}) {
  const routePrefix = hooks.routePrefix ?? '/api/feishu-bot';
  if (typeof routePrefix !== 'string' || !/^\/api\/[a-zA-Z0-9_-]+(?:\/[a-zA-Z0-9_-]+)*$/.test(routePrefix)) throw new Error('Invalid management route prefix');
  const sharedTunnel = hooks.sharedTunnel;
  if (sharedTunnel && ['appIdEnv', 'appSecretEnv', 'verificationToken', 'encryptKey'].some(key => !config[key])) throw new Error('Additional bots require independent credential references');
  ...
  if (sharedTunnel) {
    const primaryRefs = new Set(Object.values(credentialRefs.get(sharedTunnel) ?? {}));
    const ownRefs = ['appId', 'appSecret', 'verificationToken', 'encryptKey'].map(key => refs[key]);
    if (new Set(ownRefs).size !== ownRefs.length || ownRefs.some(ref => primaryRefs.has(ref))) throw new Error('Additional bots require independent credential references');
  }
  ...
  const assertOpen = () => { if (closing) throw new ManagementError(503, '机器人管理服务正在关闭'); };
  const tunnelConfig = hooks.isTunnelRequired ? new Proxy(config, { get(target, key) { return key === 'connectionMode' ? (hooks.isTunnelRequired() ? 'webhook' : 'websocket') : target[key]; } }) : config;
  const controlTunnel = (action, expectedProvider) => {
    assertOpen();
    if (sharedTunnel) throw new ManagementError(409, '隧道由所有机器人共享，请在默认机器人设置中管理隧道');
  ...
  const service = {
    checkSetup() {
      ...
      const operation = writes.then(() => {
        assertOpen();
        return checkSetup({ ctx, config, service, getConnectionStatus: hooks.getConnectionStatus });
      });
      writes = operation.catch(() => {});
      return operation;
    },
    repairSetup() {
      ...
      const operation = writes.then(() => {
        assertOpen();
        return repairSetup({ ctx, config, service: { ...service, async startTunnel() {
          assertOpen();
          if (sharedTunnel) throw new ManagementError(409, '请在默认机器人管理共享隧道');
          const status = await supervisor.start();
          return { success: !['error', 'unsupported', 'not_required'].includes(status.state), status };
        } }, getConnectionStatus: hooks.getConnectionStatus });
      });
      writes = operation.catch(() => {});
      return operation;
    },
    async getConfig() {
      const sharedConfig = sharedTunnel ? await sharedTunnel.getConfig() : null;
      const values = Object.fromEntries(await Promise.all(Object.keys(refs).map(async key => [key, sharedTunnel && key === 'ngrokAuthtoken' ? '' : await resolve(key)])));
      if (sharedTunnel) values.ngrokAuthtoken = sharedConfig.configured.ngrokAuthtoken ? true : '';
      return {
        ...Object.fromEntries(PUBLIC_FIELDS.map(key => [key, (sharedTunnel && TUNNEL_FIELDS.includes(key) ? sharedConfig[key] : config[key]) ?? ''])),
        ...(sharedTunnel ? { sharedTunnel: true } : {}),
        serverTunnelRequired: sharedTunnel ? sharedConfig.serverTunnelRequired : Boolean(hooks.isTunnelRequired?.() ?? config.connectionMode === 'webhook'),
        ...
        capabilities: { scheduledTasks: false, ngrokProcessControl: !sharedTunnel, tunnelProcessControl: !sharedTunnel },
      };
    },
    updateConfig(input) {
      assertOpen();
      ...
      const run = async () => {
        ...
        if (sharedTunnel) {
          const sharedConfig = await sharedTunnel.getConfig();
          for (const key of TUNNEL_FIELDS) {
            if (!(key in updates)) continue;
            if (updates[key] !== sharedConfig[key]) throw new ManagementError(400, '隧道配置由所有机器人共享，请在默认机器人设置中修改');
            delete updates[key];
          }
          if (typeof input.ngrokAuthtoken === 'string' && input.ngrokAuthtoken.trim()) throw new ManagementError(400, '请在默认机器人设置中修改 ngrok Authtoken');
        }
        ...
        await hooks.beforeUpdate?.(input, updates);
        ...
        await service.reconcileTunnel();
        return { success: true };
      };
      const operation = hooks.serializeUpdate ? hooks.serializeUpdate(() => previous.then(run)) : previous.then(run);
      writes = operation.catch(() => {});
      return operation;
    },
    ...
    async getNgrokStatus() {
      if (sharedTunnel) return sharedTunnel.getNgrokStatus();
      try {
    ...
    reconcileTunnel() { return sharedTunnel ? Promise.resolve() : supervisor.reconcile(); },
    dispose() {
      if (!disposal) {
        closing = true;
        for (const dispose of routeDisposers) dispose?.();
        disposal = writes.then(() => sharedTunnel ? undefined : supervisor.dispose());
      }
      return disposal;
    },
    async getTunnelStatus() {
      if (sharedTunnel) {
        if (config.connectionMode === 'websocket') return { ...(await sharedTunnel.getTunnelStatus()), mode: 'websocket', state: 'not_required', running: null, url: null, message: '长连接无需公网隧道', sharedTunnel: true };
        return { ...(await sharedTunnel.getTunnelStatus()), mode: config.connectionMode, sharedTunnel: true };
      }
      const common = { provider: config.tunnelProvider, mode: tunnelConfig.connectionMode, managed: false, port: ctx.webServer.port };
      if (tunnelConfig.connectionMode === 'websocket') return { ...common, state: 'not_required', running: null, url: null, message: '长连接无需公网隧道' };
    ...
    async getPublicBaseUrl() {
      if (sharedTunnel) return sharedTunnel.getPublicBaseUrl();
    ...
    async getWebhookUrl() {
      ...
      const base = await service.getPublicBaseUrl();
      return base ? `${base}${config.path || '/webhook/feishu'}` : null;
    },
  };
  credentialRefs.set(service, refs);
  if (!sharedTunnel) supervisor = hooks.tunnelSupervisor ?? createTunnelSupervisor({ config: tunnelConfig, getPort: () => ctx.webServer.port, resolveNgrokToken: () => resolve('ngrokAuthtoken'), detectNgrok: () => service.getNgrokStatus(), logger: ctx.logger });
  ctx.effect(() => () => service.dispose(), 'feishu-bot: management cleanup');
  const routes = {
    ...
    '/setup/check': { POST: () => service.checkSetup() },
    '/setup/repair': { POST: () => service.repairSetup() },
