...
const AUTH_METHODS = new Set(['PLAIN', 'LOGIN', 'CRAM-MD5', 'XOAUTH2']);
...
export class SmtpMailError extends Error {
  constructor(code, message, { uncertain = false, smtpCode, enhancedCode, authMethod } = {}) {
    super(message);
    this.name = 'SmtpMailError';
    this.code = code;
    this.uncertain = uncertain;
    if (Number.isInteger(smtpCode) && smtpCode >= 400 && smtpCode <= 599) this.smtpCode = smtpCode;
    if (typeof enhancedCode === 'string' && /^[45]\.\d{1,3}\.\d{1,3}$/.test(enhancedCode)) this.enhancedCode = enhancedCode;
    if (AUTH_METHODS.has(authMethod)) this.authMethod = authMethod;
  }
}
...
function safeFailure(error, sending) {
  ...
  if (error?.code === 'EAUTH') {
    // Preserve only protocol status and a known mechanism. Never attach raw
    // responses/commands: either may echo a credential or its base64 form.
    const enhancedCode = typeof error.response === 'string' ?
      /^[45]\d{2}[ -]([45]\.\d{1,3}\.\d{1,3})(?:\s|$)/.exec(error.response.slice(0, 64))?.[1] : undefined;
    const authMethod = typeof error.command === 'string' && /^AUTH (?:PLAIN|LOGIN|CRAM-MD5|XOAUTH2)$/.test(error.command) ? error.command.slice(5) : undefined;
    return new SmtpMailError('AUTH_FAILED', 'SMTP authentication did not complete', { smtpCode: error.responseCode, enhancedCode, authMethod });
  }
