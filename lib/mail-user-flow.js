...
const DISCOVERY_RETRY = '暂时无法确认可用的邮箱服务配置，尚未进行授权码验证，也未发送邮件。已保留本次待发送任务；可以稍后发送 /mail retry 让我重新查找，无需填写服务器或端口。';
...
export function createMailUserFlow(ctx, { verify = verifySmtp, discover = discoverMailProvider, probe = probeSmtp, now = () => new Date() } = {}) {
    ...
    discover(identity, { chatId, signal } = {}) {
      return serial(identity, async ref => {
        try {
          ...
          signal?.throwIfAborted();
          if (found.status !== 'found') return { handled: true, reply: DISCOVERY_RETRY };
          let selected, source;
          ...
          if (!selected) return { handled: true, reply: DISCOVERY_RETRY };
          signal?.throwIfAborted();
          ...
          signal?.throwIfAborted();
          return { handled: true, reply: DISCOVERY_RETRY };
        }
      });
    },
