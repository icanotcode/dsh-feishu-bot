...
const EMAIL_PROMPT = '请提供你本人的发件邮箱地址（只发送一个邮箱地址），我会自动选择已知邮箱提供商的 SMTP 服务，然后再询问授权码。';
  ...
  async function read(ref) {
    ...
    try {
      ...
      if (!state.smtp && ['awaiting_server', 'awaiting_code'].includes(state.stage)) {
        ...
        if (selected) { state.smtp = selected; state.stage = 'awaiting_code'; }
      }
      return state;
      ...
      return serial(identity, async ref => {
        ...
        try {
          ...
          if (state.stage === 'awaiting_email') {
            ...
            const next = { ...state, stage: selected ? 'awaiting_code' : 'awaiting_server', email: address, ...(selected ? { smtp: selected } : {}) };
            await write(ref, next, signal);
            return done(selected ? `已自动选择邮箱服务。${codePrompt(next)}` : `未能可靠识别此邮箱的 SMTP 服务。${SERVER_PROMPT}`);
          }
