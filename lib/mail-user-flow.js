...
function verificationFailure(error) {
  switch (error?.code) {
    case 'AUTH_FAILED': {
      const status = Number.isInteger(error.smtpCode) && error.smtpCode >= 400 && error.smtpCode <= 599 ? error.smtpCode : undefined;
      const detail = status ? `（SMTP ${status}${typeof error.enhancedCode === 'string' && /^[45]\.\d{1,3}\.\d{1,3}$/.test(error.enhancedCode) ? ` / ${error.enhancedCode}` : ''}${['PLAIN', 'LOGIN', 'CRAM-MD5', 'XOAUTH2'].includes(error.authMethod) ? `，AUTH ${error.authMethod}` : ''}）` : '';
      const guidance = status && status < 500 ? '服务器暂时拒绝认证，请稍后重试，并检查邮箱是否有安全提醒或访问频率限制。' :
        [500, 501, 502, 504].includes(status) ? '服务器未接受当前认证命令或方式，需要检查 SMTP 服务兼容性。' :
        status === 534 ? '服务器要求的认证条件未满足，请检查邮箱的第三方客户端访问与安全设置。' :
        '请检查邮箱的第三方客户端访问与安全提醒。若确认邮箱和授权码匹配，不能仅凭此错误判断授权码有误。';
      return `SMTP 身份验证失败${detail}。${guidance}`;
    }
