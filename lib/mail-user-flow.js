...
import { verifySmtp, validateSmtpConfig } from './smtp-mail.js';

const EMAIL = /^[A-Za-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[A-Za-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?$/;
const STAGES = new Set(['awaiting_email', 'awaiting_server', 'awaiting_code', 'awaiting_recipient', 'ready']);
const PRIVATE_ONLY = '邮箱配置只支持与机器人的私聊，请在私聊中发送 /mail。不要在群聊中发送邮箱授权码。';
const EMAIL_PROMPT = '请提供你本人的发件邮箱地址（只发送一个邮箱地址），之后配置该邮箱提供商的 SMTP 服务。';
...
function codePrompt(state) {
  ...
  return `当前使用 SMTP 服务（${serverLabel(state)}）。请在当前私聊中发送 /mail code 授权码。请使用当前邮箱提供商生成的 SMTP 授权码，不要发送邮箱登录密码或飞书应用 App Secret。`;
}
...
function prompt(state) {
  return state.stage === 'awaiting_email' ? EMAIL_PROMPT : state.stage === 'awaiting_server' ? SERVER_PROMPT : state.stage === 'awaiting_code' ? codePrompt(state) : RECIPIENT_PROMPT;
}
...
export function createMailUserFlow(ctx, { verify = verifySmtp, now = () => new Date() } = {}) {
  ...
  return {
    ...
    getAccount(identity) {
      return serial(identity, async ref => {
        const state = await read(ref);
        return state?.stage === 'ready' ? { config: smtp(state), password: state.password, recipient: state.recipient, chatId: state.chatId } : null;
      });
    },
      ...
      return serial(identity, async ref => {
        ...
        try {
          ...
          if (/^\/mail\s+server(?:\s|$)/i.test(command)) {
            ...
            const next = { stage: 'awaiting_code', chatId, email: state.email, smtp: server };
          ...
          if (/^\/mail\s+code(?:\s|$)/i.test(command) || secret !== undefined) {
            if (state?.stage === 'awaiting_server' || (state?.stage === 'awaiting_code' && !state.smtp)) return done(SERVER_PROMPT);
            ...
            try { signal?.throwIfAborted(); await verify(smtp(state), secret, { signal }); }
            catch (error) {
              signal?.throwIfAborted();
          ...
          if (isCommand) return done('邮箱命令：/mail、/mail server 服务器地址 端口 tls|starttls、/mail code 授权码、/mail status、/mail to 收件邮箱、/mail reset、/mail cancel。');
          if (!state) {
          ...
          if (state.stage === 'awaiting_server') return done(SERVER_PROMPT);
          ...
          if (state.stage === 'awaiting_email') {
            await write(ref, { stage: 'awaiting_server', chatId, email: address }, signal);
            return done(SERVER_PROMPT);
          }
