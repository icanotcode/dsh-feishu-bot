    ...
    try {
      database.exec(`
        ...
        CREATE TABLE IF NOT EXISTS email_sends (
          id TEXT PRIMARY KEY, createdAt TEXT NOT NULL, result TEXT
        );
...
function userStore(database, checkStorage) {
  ...
  const api = {
    getEmailSend(id) {
      const value = statement('SELECT createdAt, result FROM email_sends WHERE id = ?').get(required(id, 'id'));
      return value ? { createdAt: value.createdAt, result: value.result ? JSON.parse(value.result) : null } : null;
    },
    claimEmailSend(id) {
      return statement('INSERT OR IGNORE INTO email_sends(id, createdAt) VALUES (?, ?)').run(required(id, 'id'), iso()).changes === 1;
    },
    completeEmailSend(id, result) {
      statement('UPDATE email_sends SET result = ? WHERE id = ?').run(JSON.stringify(result), required(id, 'id'));
    },
    getProfile,
