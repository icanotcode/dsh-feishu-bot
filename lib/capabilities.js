...
export const capabilityTool = {
  ...
  timeoutMs: 20000,
  description: 'Invoke capabilities granted to you as a Feishu user. action=list shows your granted capabilities; action=matrix (admin only) audits every capability with its roles and group; action=load returns a capability\'s instructions (follow them exactly); action=run executes one of its declared commands (name from the instructions, args per its declared schema). Never attempt unlisted capabilities or undeclared commands.',
  parameters: object({
    action: { type: 'string', enum: ['list', 'load', 'run', 'matrix'] },
...
const granted = (manifest, roles) => roles.includes(SUPER_ROLE)
  ...
  || (manifest.group && roles.includes(manifest.group));
  ...
  return {
    ...
    async execute(args, binding) {
      ...
      if (args.action === 'matrix') {
        if (!roles.some(r => r === SUPER_ROLE || r === 'admin')) throw new Error('权限矩阵仅管理员可见。');
        const all = await scanCapabilities(defaultRoles);
        return { capabilities: all.map(cap => ({
          name: cap.name,
          roles: cap.manifest.roles || `(默认:${defaultRoles.join(',')})`,
          group: cap.manifest.group || null,
          auto: Boolean(cap.manifest.auto),
        })) };
      }
      const caps = await grantedCapabilities(roles, defaultRoles);
