        ...
        loading ? h('p', { role: 'status' }, '正在读取配置…') : loadError
          ...
          : h(React.Fragment, null,
            h('form', { onSubmit: event => {
              ...
              h('section', null,
                h('h3', null, '个人邮箱（SMTP）'),
                h('p', { className: 'feishu-muted' }, '每位飞书用户独立绑定自己的邮箱。普通聊天无需配置；首次要求发邮件时，skill 会引导完成邮箱、SMTP 授权码和收件邮箱配置。'),
                h('p', null, '在机器人私聊中发送 ', h('code', null, '/mail'), ' 开始，按提示依次提供本人邮箱、SMTP 服务器与加密方式、授权码和目标收件邮箱。'),
                h('p', { className: 'feishu-muted' }, h('code', null, '/mail status'), ' 查看状态；', h('code', null, '/mail to 收件邮箱'), ' 更换收件人；', h('code', null, '/mail reset'), ' 清除自己的邮箱绑定。'),
                h('p', { className: 'feishu-muted' }, '支持自定义 SMTP 服务器、端口及 TLS／STARTTLS，需邮箱服务支持授权码认证；不读取收件箱。授权码在进入模型和普通历史前由插件截取；飞书聊天记录本身不由插件清除。')),
