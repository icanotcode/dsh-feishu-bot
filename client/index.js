        ...
        loading ? h('p', { role: 'status' }, '正在读取配置…') : loadError
          ...
          : h(React.Fragment, null,
            h('form', { onSubmit: event => {
              ...
              h('section', null,
                h('h3', null, '个人邮箱（SMTP）'),
                h('p', { className: 'feishu-muted' }, '每位飞书用户独立绑定自己的邮箱。首次要求发邮件时，输入邮箱后自动匹配已知 SMTP 服务，再验证授权码并继续当前发送任务；普通聊天无需配置。'),
                ...
                h('p', { className: 'feishu-muted' }, '未知企业域名需手动设置 SMTP 参数，也支持手动覆盖自动匹配结果；邮箱服务须支持授权码认证，不支持 OAuth 登录或收件箱读取。授权码在进入模型和普通历史前由插件截取；飞书聊天记录本身不由插件清除。')),
