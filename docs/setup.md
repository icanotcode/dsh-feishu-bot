...
两种方式均通过 `im.message.receive_v1` 接收文本、图文和支持的附件，功能范围相同。修改下拉选项后必须点击 **保存配置** 才生效；飞书后台也要选择相同方式。长连接模式下，HTTP Webhook 入口返回 `409`，两个入口不会同时接收消息。
...
新用户采用插件的全局权限预设，仅支持只读或工作区写入。旧配置中已有的个人权限覆盖会继续保留，但不再作为准入名单。输入文件可通过飞书上传，或由本机管理员放入对应用户子目录；远程用户没有通用 Shell、任意主机文件读取或通用 MCP 权限。数据库分开本身不是操作系统级账号或虚拟机隔离。
...
同一用户同一时间最多拥有一个当前可见的 Harness 会话。同一聊天中的追问默认复用该会话；切换到其他群聊或私聊时，先等待旧任务完成并归档上一个聊天的会话，再创建新上下文，避免混用聊天内容。不同用户不会共享会话；历史工具仍按当前用户、当前聊天隔离。
