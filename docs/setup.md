...
运行环境需 Node.js **22.13 或更高版本**，历史数据库使用内置 `node:sqlite`。文中旧版界面截图未包含新增的授权与每日上下文切换字段。
...
5. 填写应用凭据、实际存在的用户工作目录根路径和 Agent 预设，再按下文选择接收方式。每位授权用户会使用根路径下的独立子目录，请选择专用空目录。
...
完成 **版本管理与发布** 中的创建版本、发布或审批流程，确认应用对测试用户可见。群聊测试前将机器人加入群，并 @机器人发送文本。插件默认拒绝授权名单之外的普通任务。应用可用范围不能替代插件授权名单；事件接通后按下文使用 `/whoami` 并添加用户。
...
| 授权用户 | 每行填写 `open_id 显示名称`，例如 `ou_example 张三`；显示名称可包含空格。空名单拒绝所有普通任务。 |
...
1. 事件订阅和回复权限生效后，在飞书私聊机器人发送 `/whoami`。这个命令不调用 Agent，未授权用户也可查询自己的身份。
2. 在本机插件设置中，将返回的 `open_id` 和自己指定的显示名称填入授权名单，例如 `ou_example 张三`，然后保存。不要用姓名、邮箱或其他应用的用户 ID 替代该 `open_id`。
3. 名单下方可为每位用户选择「继承默认权限」「只读」或「工作区写入」；继承默认时使用全局权限预设。更改显示名称保留同一 open_id 的权限，删除该用户同时删除其覆盖设置。每个用户分别查询自己的身份并由管理员添加。显示名称决定 Harness 会话名称，隔离边界使用稳定的租户与用户身份，改名不会把历史归给别人。
4. 保持权限为只读或工作区写入。需要输入文件时，由本机管理员放入对应用户子目录；远程用户没有通用 Shell、任意主机文件读取或通用 MCP 权限。数据库分开本身不是操作系统级账号或虚拟机隔离。

| 飞书命令 | 行为 |
| --- | --- |
| `/whoami` | 查询自己的身份，未授权用户也可使用，便于申请加入名单。 |
| `/help` | 查看插件命令说明。 |
| `/status` | 查看当前聊天的会话状态。 |
...
每位用户按租户与稳定用户身份使用独立 SQLite 数据库，记录消息时间戳及会话关联；Agent 历史工具进一步限制为**当前用户、当前聊天**。可通过自然语言请求搜索、读取、新增笔记、更正或软删除历史，例如“查找本聊天上个月讨论的计划，并给出时间”。来自另一个群的内容不会因属于同一用户就自动暴露到当前群。
...
3. 先确认发送者已通过 `/whoami` 加入插件授权名单，再发送 `请只回复：飞书连接正常`。这是一条普通文本请求，不是插件命令。
4. 确认 Harness 出现包含管理员所填显示名称的会话，并生成答复。
5. 确认机器人在飞书回复了这次请求。若需要群聊，再单独测试群内 @机器人。
6. 开通表情写权限后，用一条耗时稍长的文本任务检查原消息上的敲键盘表情是否在处理期间出现、结束后移除；快速完成的任务可能不易观察。
7. 继续发送追问，确认使用同一会话；发送 `/new` 后再提问，确认使用新会话。让两个已授权用户分别发送消息，确认名称、上下文、文件目录和历史互不混用。
8. 请求 Agent 查找刚才的历史记录及时间，检查当前聊天之外的记录无法访问。未授权账号普通消息应被拒绝。
...
**从 0.1.4 升级时：**先确认 Node.js 至少为 22.13；旧配置中的 `danger-full-access` 必须改为 `read-only` 或 `workspace-write`。新授权名单默认为空，普通任务会被拒绝，需通过 `/whoami` 查询并由本机管理员填写名单。远程会话不再开放旧版的通用工具能力，旧会话不会自动合并或导入数据库。
