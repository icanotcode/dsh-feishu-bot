...
| 角色 | 能力可见性 | 审批权 |
|---|---|---|
| `superadmin` 超管 | 全部能力（通配，不受技能 roles 限制） | 可授予任何角色（含 admin） |
| `admin` 管理 | 按技能 roles 匹配 | 只能授予 member 及以下，不能造出同级/上级 |
| `member` 普通用户 | 按技能 roles 匹配 | 无 |

判定算法：用户角色含 `superadmin` → 全部可见；否则能力可见当且仅当
技能 `roles` 与用户角色有交集，**或技能的 `group` 在用户角色中**；命令级 `roles` 同理单独收紧。
未登记的自定义角色按普通用户级别处理。

防角色爆炸（技能增多时的收敛规则）：
优先用固定档位（admin/member）；同域技能 ≥3 个时在 `feishu.json` 声明 `group`，
用户角色授组不授单技能；单技能领域角色（xxx-user）仅作例外。
管理员用 `feishu_capability` 的 `matrix` 动作审计全部技能的角色与分组。
