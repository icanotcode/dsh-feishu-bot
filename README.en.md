...
### Send through your personal mailbox

Complete name confirmation first. Ordinary conversation does not require an email binding. Your first request to send email, or `/mail` in a direct chat with the bot, starts setup: follow **your mailbox → automatic SMTP provider selection → authorization code → continue sending**. Known providers use a quick match; for unknown domains the Agent calls the dedicated `feishu_mail_discover` tool to query DNS mail-submission records, the domain’s official HTTPS autoconfiguration, and Thunderbird ISPDB. If standard discovery finds no usable settings, the Agent searches official provider documentation through Harness’s configured search provider, reads page text, and extracts SMTP settings. The plugin checks the page’s relationship to the mailbox domain and verifies the quoted configuration before probing TLS/SMTP connectivity without credentials and requesting your authorization code. Users do not need to enter a server, port, or encryption mode. Web search requires an administrator-configured, working Harness search provider. If search is unavailable, no reliable configuration is available, or the network is unreachable, setup explains that it cannot finish yet and keeps the pending send request; `/mail retry` tries discovery again. Discovery cannot identify every mailbox. `/mail server` remains an advanced manual override. Submit your code directly in the private chat or use `/mail code YOUR_CODE` when prompted. Plain text sent while waiting for the code is also intercepted before reaching the Agent. Authentication checks SMTP login only; it does not send a test email.

When an explicit send request starts setup, the plugin retains that current request and any recipient already specified. Once verified, it continues the request, asking only for missing recipient, subject, or body details. You do not need to repeat the send instruction. Starting `/mail` manually only configures the account: it does not send an email without a current send request or replay old drafts. The sender and recipient come from your binding; the sending tool cannot override that account or recipient. Use `/mail status` to check the binding, `/mail to ADDRESS` to change the recipient, `/mail reset` to delete your binding, or `/mail cancel` to cancel unfinished setup while retaining an already-ready binding. Canceling or resetting discards the pending send request. Bindings are isolated by bot, tenant, and stable user ID, independently of display names; pending setup requests are also tied to the current private chat.

The plugin reads `skills/feishu-assistant/SKILL.md` and adds its instructions to each Feishu message entering the Agent; no separate Skill installation is required. Discovery and search use only the mailbox domain; search adds fixed keywords for official SMTP settings. Your full address and authorization code are never sent to lookup services. MX records only help establish a provider relationship and are never used directly as submission servers. Web pages are configuration data, not executable instructions; validation uses the official source URL and supporting excerpt in a temporary cache (10-minute lifetime; research is repeated after restart). Private-network destinations and redirect bypasses are rejected; server certificates are verified. This thin Skill supplies conversation and mail-tool guidance; the plugin's entry state machine handles name confirmation, email setup, and authorization-code protection. Codes are stored in Harness credentials and redacted before Webhook delivery, history, or model input. **The plugin does not remove the original messages retained by Feishu itself.**

The SMTP host, port and TLS/STARTTLS mode are configured per bot and user, with mandatory certificate validation and authorization-code authentication. Providers requiring OAuth login are not supported. Changing servers clears the previous code and recipient and requires authentication again. Existing server settings remain usable; incomplete bindings without a server keep the mailbox address and enter automatic discovery, without asking ordinary users for SMTP settings. IMAP/inbox access is not implemented. Mail tools require a name-confirmed user with an active direct-chat task. `feishu_send_email` accepts a subject, text, and up to 10 paths within that user's directory; the estimated MIME message limit is 10 MiB and the text limit is 1 MiB. SQLite-backed deduplication protects repeated sends with the same message and content. **SMTP acceptance is not proof of delivery**; uncertain failures are not automatically retried. Real external-email delivery has not yet received end-to-end acceptance testing. See the [setup guide (Chinese)](docs/setup.md#个人邮箱发信) for the commands and details.
