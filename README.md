...
| 个人邮箱发信 | 私聊绑定本人邮箱、自定义 SMTP 服务、授权码和固定收件邮箱后，可按明确请求发送正文及个人目录附件；不支持收信 |
...
完成姓名确认后，普通聊天无需绑定邮箱。首次要求发送邮件时，或主动在机器人私聊发送 `/mail`，才进入绑定流程：依次提供 **本人邮箱 → SMTP 服务器、端口及加密方式 → 授权码 → 目标收件邮箱**。使用 `/mail server 服务器 端口 tls或starttls` 设置邮箱服务商提供的 SMTP 参数，再按提示使用 `/mail code 授权码` 提交授权码；等待授权码时发送的普通文本也由入口拦截，不会作为任务交给模型。认证只验证 SMTP 登录，不会发送测试信。

绑定完成后，重新明确请求“把这份总结发到我绑定的邮箱”，即可发送正文及个人目录内的附件。绑定前的发送指令不会自动重放。发件人与收件人固定取自当前用户的绑定，Agent 不能自行指定账号或更换收件人。使用 `/mail status` 查看状态、`/mail to 新邮箱地址` 修改收件人、`/mail reset` 删除自己的绑定、`/mail cancel` 取消未完成配置（保留已就绪绑定）。不同机器人、租户和用户分别保存邮箱资料，姓名相同不会混用授权。
