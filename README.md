...
| 个人邮箱发信 | 私聊绑定本人飞书邮箱、SMTP 授权码和固定收件邮箱后，可按明确请求发送正文及个人目录附件；不支持收信 |
...
完成姓名确认后，普通聊天无需绑定邮箱。首次要求发送邮件时，或主动在机器人私聊发送 `/mail`，才进入绑定流程：依次提供 **本人的飞书邮箱 → SMTP 授权码 → 目标收件邮箱**。按提示使用 `/mail code 授权码` 提交授权码；等待授权码时发送的普通文本也由入口拦截，不会作为任务交给模型。认证只验证 SMTP 登录，不会发送测试信。
...
仅支持飞书 SMTP 发信，不支持 IMAP 收信。结果为 **SMTP 服务器接受**，不代表邮件已送达；结果不确定时不会自动重发。目前尚未完成真实外部邮箱的端到端收件验收。命令、附件限制与设置细节见 [个人邮箱发信](docs/setup.md#个人邮箱发信)。
