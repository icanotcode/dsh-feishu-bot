...
「任务处理」和「添加机器人」使用相同的搜索下拉框：点击 **搜索项目**，输入项目名称或路径，下方列表会实时筛选；选择后再保存配置或创建机器人。列表包含 Harness 已添加的项目和机器人已绑定目录，不会遍历磁盘。被其他机器人占用或无法访问的目录不可选。找不到目标时，先在 Harness 中添加该项目，再刷新项目列表。选择项目不会扩大远程用户权限，每位用户仍只能访问所选目录下自己的 `.feishu-users/<身份哈希>/`。
