...
| 个人邮箱发信 | 私聊输入本人邮箱，自动匹配已知 SMTP 服务，验证授权码后继续当前发信任务；支持自定义服务及个人目录附件，不支持收信 |
...
完成姓名确认后，普通聊天无需绑定邮箱。首次要求发送邮件时，或主动在机器人私聊发送 `/mail`，才进入绑定流程：**本人邮箱 → 自动匹配 SMTP 服务 → 授权码 → 继续发送**。已知邮箱服务商的服务器、端口和加密方式会自动选好，无需手填；无法识别的企业域名才询问 SMTP 参数，也可用 `/mail server 服务器 端口 tls或starttls` 手动覆盖。按提示直接发送授权码或使用 `/mail code 授权码`；等待授权码时发送的普通文本由入口拦截，不会作为任务交给模型。认证只验证 SMTP 登录，不会发送测试信。
...
插件读取 `skills/feishu-assistant/SKILL.md`，将指引加入每条进入 Agent 的飞书消息，无需另行安装 Skill。这份轻量 Skill 提供聊天与邮件工具指引；姓名确认、邮箱配置和授权码保护由插件入口的状态机处理。授权码保存到 Harness credentials，并在进入 Webhook 投递、历史与模型前脱敏；**插件不会删除飞书平台本身保留的聊天记录**。
