...
实际设置界面，拍摄于独立演示环境，未填写应用凭据。截图展示旧版 0.1.3 的 Cloudflare 接入配置；当前入口已移至 Plugin list 卡片内，截图也未包含 0.2.0 的用户授权与上下文切换设置；示例域名不代表公网连通。
...
| 文本消息处理 | 接收 `im.message.receive_v1`；同一租户、授权用户和聊天复用当前 Harness 会话 |
...
| 用户隔离 | 默认拒绝未授权用户；每用户独立工作目录及 SQLite 数据库，不同聊天上下文分开 |
| 会话管理 | 支持 `/new`、`/whoami`、`/help`、`/status`；按管理员填写的显示名称命名会话 |
| 历史记录 | 保存消息时间戳，Agent 可查询、读取、创建笔记、修改及软删除当前用户在当前聊天的记录 |
| 每日切换 | 默认 Asia/Macau 凌晨 4 点切换上下文，运行中的任务完成后切换，保留历史记录 |

**远程飞书会话仅开放受限的工作区文件工具与个人历史工具**，不开放任意 Shell、通用 MCP 或可能跨用户访问数据的飞书 API。工作区写入权限只允许操作该用户子目录；只读模式禁止修改工作区文件。管理员可为每个授权用户单独选择只读或工作区写入，未指定时继承全局权限预设。管理员控制本机 Harness 和数据目录，不属于远程用户隔离边界。
...
先在飞书私聊发送 `/whoami`，取得当前应用下的 `open_id`。管理员在本机插件设置的「授权用户」中填写一行 `open_id 显示名称` 并保存；**名单为空时所有普通任务均被拒绝**，`/whoami` 仍可用于查询自己的身份。

授权后发送简单文本，确认 Harness 中出现包含该显示名称的会话，并在飞书收到最终答复。再发送追问验证同一聊天沿用会话，发送 `/new` 验证切换。群聊测试需先加入机器人并 @机器人。
...
| 授权用户 | 空名单，默认拒绝；由管理员配置 `open_id` 和显示名称 |
...
数据库按租户与用户的稳定身份分开，不以可更改的显示名称作为身份依据。同一用户的不同群聊和私聊分别维护上下文；Agent 历史工具仅能访问当前用户、当前聊天的记录。历史包含时间戳，保存在本机 SQLite 数据库中。
...
**从 0.1.4 升级：**默认空授权名单会拒绝原来可以直接发起的任务；先通过 `/whoami` 获取身份，再由本机管理员添加名单。旧的完全访问权限不再支持，请改成只读或工作区写入。远程会话也不再拥有通用 Shell、MCP 和跨用户飞书工具。旧版逐消息会话不会自动合并为新会话或导入历史库。
