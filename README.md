...
完成姓名确认后，普通聊天无需绑定邮箱。首次要求发送邮件时，或主动在机器人私聊发送 `/mail`，才进入绑定流程：**本人邮箱 → 自动匹配 SMTP 服务 → 授权码 → 继续发送**。已知邮箱快速匹配，未知域名由 Agent 调用专用 `feishu_mail_discover` 工具在线查询 DNS 邮件提交记录、域名官方 HTTPS 自动配置和 Thunderbird ISPDB，并在不提交授权码的情况下探测 TLS／SMTP 连接。普通用户无需填写服务器、端口或加密方式；找到可用配置后才询问授权码。若没有可靠配置或网络不可达，会说明暂时无法完成并保留当前发送任务，可用 `/mail retry` 重查，不保证任意邮箱都能识别。`/mail server` 仅作为高级手动覆盖保留。按提示直接发送授权码或使用 `/mail code 授权码`；等待授权码时发送的普通文本由入口拦截，不会作为任务交给模型。认证只验证 SMTP 登录，不会发送测试信。
...
插件读取 `skills/feishu-assistant/SKILL.md`，将指引加入每条进入 Agent 的飞书消息，无需另行安装 Skill。在线发现只使用邮箱域名，不向配置查询服务提交完整邮箱或授权码，也不把收信 MX 记录直接当成发信服务器。拒绝访问私网地址及重定向绕过，连接始终验证证书。这份轻量 Skill 提供聊天与邮件工具指引；姓名确认、邮箱配置和授权码保护由插件入口的状态机处理。授权码保存到 Harness credentials，并在进入 Webhook 投递、历史与模型前脱敏；**插件不会删除飞书平台本身保留的聊天记录**。
